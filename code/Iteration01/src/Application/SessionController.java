...
public class SessionController {
  ...
  /**
   * Passes the username and password on to the Domain.UserManager, initialises the current session by setting the
   * appropriate user.
   *
   ...
   * @post Domain.User is set in session
   */
  public Role login(String username, String password) throws LoginException {
    if (getSession().isLoggedIn()) {
      throw new LoginException("Incorrect permission: Domain.User already logged in");
    }
